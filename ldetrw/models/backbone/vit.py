...
class ViT(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # maskの処理
        # mask: (B, H_orig, W_orig) → (B, 1, H_orig, W_orig) に変換してinterpolate
        mask = mask.unsqueeze(1).float()  # (B, 1, H_orig, W_orig)
        mask = F.interpolate(mask, size=(H, W), mode='nearest').to(torch.bool)
        # mask: (B, 1, H, W) → (B, H, W)
        mask = mask.squeeze(1)
        # window分割（チャンネル次元なし）
        mask = mask.reshape(B, 4, h, 4, w).permute(0, 1, 3, 2, 4).reshape(B * 16, h * w)

        # all_masked = mask.all(dim=-1, keepdim=True)  # (B*16, 1)
        # mask = mask & ~all_masked

        out = []
        for idx, blk in enumerate(self.blocks):
            x = blk(x, mask=mask if idx in self.window_block_indexes else None)
