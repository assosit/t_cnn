...
class LongSideSquarePad(object):
    def __init__(self, sizes, size_divisor=64, fill_value=127.5):
    ...
    def __call__(self, img, target=None):
        ...
        canvas_side = int(math.ceil(long_side / self.size_divisor) * self.size_divisor)
        canvas = np.ones((canvas_side, canvas_side, 3), dtype=np.float32) * self.fill_value
        canvas[:new_h, :new_w, :] = np.array(rescaled_img, dtype=np.float32)
        result_img = F.to_pil_image(canvas.astype('uint8'))
        ...
        target["size"] = torch.tensor([canvas_side, canvas_side])

        target["valid_size"] = torch.tensor([new_h, new_w])
